...
// POST /api/meal-templates/[id]/items - Add a food item to a template
export async function POST(
  ...
  try {
    ...
    // Add the food item to the template
    const result = await pool.query(
      `INSERT INTO meal_template_items (template_id, food_item_id, portion_type, portion_count)
       VALUES ($1, $2, $3, $4)
       RETURNING id, template_id, food_item_id, portion_type, portion_count, created_at`,
      [templateId, food_item_id, portion_type, portion_count]
    );

    const templateItem = result.rows[0];
    ...
    console.error('Error adding food item to template:', error);
    return NextResponse.json(
      { error: "Failed to add food item to template" },
      { status: 500 }
    );
  }
}
